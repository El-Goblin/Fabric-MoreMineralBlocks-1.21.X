...
public class SingleBlockSphereJob implements TerrainJob{
    ...
    BlockPos.MutableBlockPos positionToModify;

    public SingleBlockSphereJob(Level level, @Nullable Player player, BlockPos center, int radius, Block toPlace) {
        this.level = level;
        this.center = center;
        this.radius = radius;
        ...
        this.player = player;
        positionToModify = new BlockPos.MutableBlockPos();

        if (level.getServer() != null) {
            protectorManager = ProtectorManager.getProtectorManager(level.getServer());
        }
    ...
    public boolean process(int maxBlockOperations) {
        ...
        if (advanceX.isEmpty() && advanceZ.isEmpty()) {
            player.sendSystemMessage(Component.literal(String.format("%d", radius)));
            return true;
        }
        return false;
    }

    private int breakX4(BlockPos.MutableBlockPos positionToModify, int[] advanceDelta) {
        int blocksBroken = 0;
        ...
        if (!level.getBlockState(positionToModify).isAir() && !protectorManager.isProtected(positionToModify)) {
            level.setBlock(positionToModify, toPlace.defaultBlockState(), 50);
            blocksBroken++;
        }
    ...
    private int processY(BlockPos.MutableBlockPos positionToModify) {
        if (blocksAdvanced <= radius) {
            positionToModify.set(center.getX(), center.getY() + blocksAdvanced, center.getZ());
            if (!level.getBlockState(positionToModify).isAir() && !protectorManager.isProtected(positionToModify)) {
                level.setBlock(positionToModify.set(center.getX(), center.getY() + blocksAdvanced, center.getZ()), toPlace.defaultBlockState(), 50);
            }
            positionToModify.set(center.getX(), center.getY() - blocksAdvanced, center.getZ());
            if (!level.getBlockState(positionToModify).isAir() && !protectorManager.isProtected(positionToModify)) {
                level.setBlock(positionToModify.set(center.getX(), center.getY() - blocksAdvanced, center.getZ()), toPlace.defaultBlockState(), 50);
    ...
    private int processX(BlockPos.MutableBlockPos positionToModify) {
        if (!advanceX.isEmpty()) {
            int blocksBroken = 0;
    ...
    private int processZ(BlockPos.MutableBlockPos positionToModify) {
        if (!advanceZ.isEmpty()) {
    ...
    private int processEquals(BlockPos.MutableBlockPos positionToModify) {
